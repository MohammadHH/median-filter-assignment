...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int numOfThreads = configuration.getNumberOfThreads();
        ...
        if (configuration.getInputType().equals(FileType.TEXT_FILE)) {
            ...
            channels = new ReadFile(numOfThreads).readTextFile(inputFile, dimensions);
        } else {
            throw new UnsupportedOperationException("Image files aren't supported yet");
        }
        // apply median filter on each channel
        short[][][] filteredChannels = new ChannelsMedianFilter(windowSize, numOfThreads).medianFilter(channels);
